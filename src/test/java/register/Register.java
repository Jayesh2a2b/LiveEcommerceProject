...
public class Register extends Base {
	...
	GiftCertificates giftCertificatesPage;
	AffiliatePage affiliatePage;
	SpecialsOfferPage specialOffersPage;
...
//		action.sendKeys(Keys.ARROW_DOWN).pause(2000).sendKeys(Keys.ENTER).build().perform();
		...
		actions = clickKeyboardkeyMultipleTimes(actions, driver, Keys.TAB, 1);
		actions = typeTextUsingActions(actions, CommonUtilities.getGenerateNewMail());
		actions = clickKeyboardkeyMultipleTimes(actions, driver, Keys.TAB, 1);
		actions = typeTextUsingActions(actions, prop.getProperty("Telephone"));
		actions = clickKeyboardkeyMultipleTimes(actions, driver, Keys.TAB, 1);
		actions = typeTextUsingActions(actions, prop.getProperty("PWD"));
		actions = clickKeyboardkeyMultipleTimes(actions, driver, Keys.TAB, 1);
		actions = typeTextUsingActions(actions, prop.getProperty("CONPWD"));
		actions = clickKeyboardkeyMultipleTimes(actions, driver, Keys.TAB, 1);
		actions = clickKeyboardkeyMultipleTimes(actions, driver, Keys.ARROW_LEFT, 1);
